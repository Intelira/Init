...
class Auth_model extends CI_Model {
  ...
  public function login($nik, $pwd){
    ...
    if ($reroles && kataSandi('verify', $pwd, $reroles['password'])) {
      ...
      // Role Key Staff
      if (in_array($reroles['roleKey'], array(1, 2))) {
        $this->db->where($filter);
      ...
      // Role Key Pasien
        ...
        // Kalau Role Key 3 : Sign In History
        if ($resultset){
  ...
  public function register($data){
    ...
    $incredentials = array('userKey' => $userKey, 'nik' => $data['nik'], 'roleKey' => 3);
    // Kata Sandi
